...
export default async function VenueMenuPageEN({
  ...
  const foodSections = await getVenueMenuSections(v, "en");
  const drinkSections = v === "lagerbaren" ? await getVenueDrinkSections("en") : [];

  return (
    <>
      <PageHero title={t.menu.title} subtitle={brand.name} accentColor={brand.accent} />
